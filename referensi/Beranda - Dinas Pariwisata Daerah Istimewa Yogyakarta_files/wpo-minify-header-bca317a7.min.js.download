...
return-1}}
return GMaps}));
var site=site_url.template_url;var template=directory_uri.stylesheet_directory_uri;var id=postID.post_id;jQuery(document).ready(function($){var loading=$('#loading');if($('#map').length){var initLat=-7.7955798,initLng=110.36948959999995,map=new GMaps({div:'#map',lat:initLat,lng:initLng,zoom:10,});var data={action:'get_coordinates',};var getLocations=function(){loading.show();$.ajax({url:get.ajax_url,type:'post',data:data,success:function(data){if(data){if(data.length>1){for(var i=0;i<data.length;i++){image=template+'/location/img/'+data[i].kategori+'.png';if(data[i].latitude){map.setZoom(10);map.setCenter(initLat,initLng);map.addMarker({lat:data[i].latitude||null,lng:data[i].longitude||null,title:data[i].nama,icon:image,infoWindow:{content:'<div class="marker-box"><img class="marker-thumb" src="'+data[i].thumbnail+'"/><div class="marker-info"><a class="title" href="'+data[i].link+'"><strong>'+data[i].nama+'</strong></a><br/>'+data[i].address+'</div>'},click:function(e){map.map.panTo(e.position)}})}}}else if(data.length===1){image=template+'/location/img/'+data[0].kategori+'.png';if(data[0].latitude){map.setZoom(13);map.setCenter(data[0].latitude,data[0].longitude);map.addMarker({lat:data[0].latitude||null,lng:data[0].longitude||null,title:data[0].nama,icon:image,infoWindow:{content:'<div class="marker-box"><img class="marker-thumb" src="'+data[0].thumbnail+'"/><div class="marker-info"><a class="title" href="'+data[0].link+'"><strong>'+data[0].nama+'</strong></a><br/>'+data[0].address+'</div>'},click:function(e){map.map.panTo(e.position)}})}}}else{alert('Data tidak ditemukan');$("#clear").click()}
loading.hide()},error:function(){alert("refresh ulang halaman")},})};$('body').on('click','#filter',function(){var kategori=$('#kategori').val(),places=$('#places').val();data={action:'get_coordinates',kategori:kategori,places:places};map.removeMarkers();getLocations()});$("#places").keyup(function(event){if(event.keyCode==13){$("#filter").click()}});$('body').on('click','#clear',function(){data={action:'get_coordinates'};$('#kategori').val('');$('#places').val('');map.removeMarkers();getLocations()});getLocations();GMaps.on('marker_added',map,function(marker){$('#markers-with-index').append('<li><a href="#" class="pan-to-marker" data-marker-index="'+map.markers.indexOf(marker)+'">'+marker.title+'</a></li>');$('#markers-with-coordinates').append('<li><a href="#" class="pan-to-marker" data-marker-lat="'+marker.getPosition().lat()+'" data-marker-lng="'+marker.getPosition().lng()+'">'+marker.title+'</a></li>')})}
if($('#map-single').length){$.ajax({url:get.ajax_url,type:'post',data:{action:'get_single_coordinates',id:id},success:function(data){if(data){for(var i=0;i<data.length;i++){if(data[i].latitude){console.log(data[i].latitude);map=new GMaps({div:'#map-single',lat:data[i].latitude||null,lng:data[i].longitude||null,zoom:15,});map.addMarker({lat:data[i].latitude||null,lng:data[i].longitude||null,click:function(e){map.map.panTo(e.position)}})}}}else{map=new GMaps({div:'#map-single',lat:-7.7955798,lng:110.36948959999995,zoom:10,})}},error:function(){map=new GMaps({div:'#map-single',lat:-7.7955798,lng:110.36948959999995,zoom:10,})},})}});
/*
